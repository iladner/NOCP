---
title: "National priorities for Colombia"
...
theme_maps <- theme(axis.text.x = element_text(size = 8),
...
local_crs <- "+proj=moll +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +units=m +no_defs"
...
```{r}
world_eezs <- sf::st_read(file.path(emlab_data_dir, "marine-regions-eez-v11","World_EEZ_v11_20191118", "eez_v11.shp")) %>%
...
eez_raster_df <- raster::as.data.frame(trim(eez_raster), xy = T) %>% filter(!is.na(layer), layer > 0)
```
...
```{r}
ggplot()+
  geom_raster(data = eez_raster_df %>% filter(layer > 0.05), aes(x, y, fill = layer))+
  geom_sf(data = eez, fill = "transparent")+
  geom_sf(data = land_shp, fill = "transparent")+
  geom_sf(data = highly_mpas, fill = "transparent", col = "red")
```
...
(protection_levels <- ocean_df %>% 
  summarise(f_highly_mpas = sum(f_highly_mpa/n()),
            ...
            f_eez_highly_mpas = sum(f_highly_mpa[f_eez > 0 & f_highly_mpa > 0.5]/eez_size, na.rm = T)) %>% 
...
## adjust all features by the fraction of pixel within EEZ

```{r}
f_eez_factor <- ocean_df$f_eez[ocean_df$f_eez > 0 & !is.na(ocean_df$f_eez)]

eez_rows <- rownames(norm_features_matrix) %in% eez_cell_ids

norm_features_matrix[eez_rows, c(bio_feature_names, stock_names)] <- sweep(norm_features_matrix[eez_rows, c(bio_feature_names, stock_names)], MARGIN = 1, f_eez_factor, `*`)
...
map_priorities <- function(rank_raster,
  ...
  prj_raster_df <- rank_raster %>%
    ...
    crop(as_Spatial(st_transform(st_buffer(eez, dist = 100000), projection))) %>%
    mask(as_Spatial(st_transform(st_buffer(eez, dist = 100000), projection))) %>%
    as.data.frame(xy = T) %>%
  ...
  map <- prj_raster_df %>%
    ...
    geom_sf(data =  land_shp,
            inherit.aes = F, fill = "wheat", col = "transparent",linetype = 2, size = 0.5)+
...
bio_map <- map_priorities(rank_raster = bio_raster, 
               priority = "Biodiversity", 
               scenario = country_name,
               projection = local_crs, 
               mpas_shp = highly_mpas,
               land_shp = land_shp) +
  geom_sf(data = highly_mpas, fill = "transparent", col = "black", inherit.aes = F)+
  coord_sf(xlim = c(min_lon,max_lon),ylim = c(min_lat, max_lat))+
   annotate("rect", xmin = -7836076-bbox_buffer, xmax = -7669382+bbox_buffer, ymin = 1784724-bbox_buffer, ymax = 1989077+bbox_buffer,fill = "transparent", col = "red",linetype = 2, size = 1)+
    annotate("rect", xmin = -7784071-bbox_buffer, xmax = -7715646+bbox_buffer, ymin = 670836-bbox_buffer, ymax = 893188+bbox_buffer,fill = "transparent", col = "red",linetype = 2, size = 1)
...
bio_summary <- bio_benefit_curve %>% 
  ...
  summarize(objective = "biodiversity",
            max_v = max(v),
            optim_protection = min(fraction_protected[v == max_v]),
...
(max_carbon_delta <- carbon_benefit_curve %>% 
    summarize(max_v = max(v),
              current_mpas_v = if_else(protection_levels$f_eez_highly_mpas <= 0, 0, 
                                       max(v[cell_id %in% protected_cell_ids])),
              max_delta_v = max_v - current_mpas_v,
              ninety_percent_delta_v = current_mpas_v + 0.9*max_delta_v,
              optim_protection = 100*max(fraction_protected[v == max_v]),
              optim_protection_90 = 100*max(fraction_protected[which.min(abs(ninety_percent_delta_v - v))])) %>% 
...
carbon_map <- map_priorities(rank_raster = carbon_raster, 
               priority = "Carbon", 
               scenario = country_name,
               projection = local_crs, 
               mpas_shp = highly_mpas,
...
food_benefit_curve %>% 
  ggplot(aes(x = fraction_protected, v))+
  geom_line()
...
(max_food_delta <- food_benefit_curve %>% 
  summarize(max_v = max(v[f_highly_mpa <= 0.5]),
            current_mpas_v = if_else(protection_levels$f_eez_highly_mpas <= 0, 0, 
                                     last(v[f_highly_mpa > 0.5])),
            max_delta_v = max_v - current_mpas_v,
            ninety_percent_delta_v = current_mpas_v + 0.9*max_delta_v,
            optim_protection = 100*max(fraction_protected[v == max_v]),
            optim_protection_90 = 100*max(fraction_protected[fraction_protected < optim_protection/100 & 
                                                            which.min(abs(ninety_percent_delta_v - v))])) %>% 
   mutate_at(vars(optim_protection_90, optim_protection), round, 2))
```
```{r, warning = F}
...
food_map <- map_priorities(rank_raster = food_raster, 
               priority = "Food", 
               scenario = country_name,
               projection = local_crs, 
               mpas_shp = highly_mpas,
...
# Final output map for report
```{r}

bio_map+carbon_map+food_map+guide_area()+plot_layout(guides = "collect")

#ggsave(file.path(fig_path, "overlap.png"))
...
triple_wins_df %>% 
  ...
  geom_sf(data =  st_transform(land_shp, 
                               crs = local_crs), 
  ...
  geom_sf(data = eez, fill = "transparent", col = "black", inherit.aes = F)+
  geom_sf(data = highly_mpas, fill = "transparent", col = "red", inherit.aes = F)+
...
equal_wts_w_vector <- calculate_multiobjective_weights(alpha_b = 1/3, alpha_c = 1/3, alpha_f = 1/3,
                                                       ...
                                                       max_b = max_bio_delta$max_delta_v,
                                                       max_c = max_carbon_delta$max_delta_v, 
...
map_priorities(rank_raster = multi_raster, 
               ...
               mpas_shp = highly_mpas,
               land_shp = land_shp) +
  #geom_sf(data = eez, inherit.aes = F, fill = "transparent")+
  geom_sf(data = highly_mpas, fill = "transparent", col = "red", inherit.aes = F)+

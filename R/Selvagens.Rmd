---
title: "National priorities for Selvagens"
...
eez <- world_eezs %>% 
  filter(geoname == "Portuguese Exclusive Economic Zone (Madeira)") %>% 
  st_union() %>% 
  st_as_sf() %>%
...
```{r}
land_shp <- rnaturalearth::ne_countries(scale = 50, country = "Portugal", returnclass = "sf") %>% 
    st_transform(crs = crs(ocean_low_res_moll))  

madeira <- land_shp %>% 
  st_crop(eez)

islands <- sf::st_read("../../Selvagens-PS/selvagens_islands.shp") %>% 
  st_transform(crs = crs(ocean_low_res_moll))

canaries <- sf::st_read("../../Selvagens-PS/canary_islands.shp") %>% 
  st_transform(crs = crs(ocean_low_res_moll))

buffers <- sf::st_read("../../Selvagens-PS/selvagens_buffers.shp") %>% 
  st_transform(crs = crs(ocean_low_res_moll))

current_reserve <-  tribble(~lon, ~lat,
                            "16 03 15", "30 01 35", 
                            "15 52 15", "30 09 10") %>% 
  separate(lat, paste("lat",c("d","m","s"), sep="_") ) %>%
  separate(lon, paste("long",c("d","m","s"), sep="_" ) ) %>%
  mutate_each(as.numeric) %>%
  transmute(lat_dec=lat_d + lat_m/60 + lat_s/60^2,
            long_dec=long_d + long_m/60 + long_s/60^2, 
            id = "reserve") %>% 
  expand(lat_dec, long_dec, id) %>% 
  mutate(order = c(1, 4,2,3),
         long_dec = long_dec*-1) %>% 
  arrange(order) %>% 
  sf::st_as_sf(coords = c("long_dec", "lat_dec"), crs = 4326) %>% 
  group_by(id) %>%
...
ggplot()+
  ...
  geom_sf(data = islands, fill = "transparent")+
  geom_sf(data = buffers, fill = "transparent")+
  geom_sf(data = highly_mpas, fill = "transparent", col = "red", size = 0.01)+
  geom_sf(data = madeira)+
  geom_sf(data = current_reserve)
```
...
map_priorities <- function(rank_raster,
  ...
  map <- prj_raster_df %>%
    ...
    labs(title = paste("Ocean Conservation Priorities", priority, sep = " "),
         subtitle = scenario)+
    easyR::ggtheme_map()
...
map_priorities(rank_raster = bio_raster, 
               ...
               land_shp = madeira) +
  geom_sf(data = buffers, fill = "transparent", col = "red",  inherit.aes = F)+
  geom_sf(data = eez, fill = "transparent", col = "black",  inherit.aes = F, linetype = 2)+
  geom_sf(data = madeira,  inherit.aes = F)+
...
triple_wins_df %>% 
  ...
  ggspatial::annotation_north_arrow(location = "tl", which_north = "true", height = unit(0.1, "native"), width = unit(0.1, "native"),
                                    pad_x = unit(0.05, "native"), pad_y = unit(0.05, "native"),
                                    style = north_arrow_fancy_orienteering)+
  labs(y = "", x = "", subtitle = "", title = "Overlap of top 30% priority areas")+
